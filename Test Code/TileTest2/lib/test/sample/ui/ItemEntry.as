package sample.ui{
	...
	public class ItemEntry extends Box {
		function ItemEntry(item:DatabaseObject, tooltip:FlxText, toolTipImage:FlxSprite, callback:Function, bought:Boolean = false) {
			super();
			...
			if (bought) {
				...
				buyButton = new TextButton("Cost: " + item.coinCost + " Coins", function(){callback(item.key, item.coinCost, item.Type, item.Image)})
			}
